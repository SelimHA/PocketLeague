...
## V32 settings, pitch size and match length notes

V32 adds:

- Custom pitch size per match/lobby: Compact, Standard, Wide, Stadium XL.
- Custom match length per match/lobby: 3, 5, 7 or 10 minutes.
- Gameplay defaults in Settings for pitch size and match length.
- Signed-in account settings sync through Firebase at `settings/{uid}`.
- Guest/anonymous users still play normally and keep settings in local browser storage only.

### Firebase steps for V32

If you deploy with GitHub/Netlify or any static host, manually publish the updated rules:

1. Open Firebase Console → Build → Realtime Database → Rules.
2. Replace the rules with this package's `database.rules.json`.
3. Click **Publish**.

The new rule section is:
...
No new database instance is needed. The app uses the existing Realtime Database and adds the `settings/{uid}` path alongside `profiles`, `leaderboard`, `lobbies`, and `voice`.

### Extra V32 patch notes

V32 also adds the later settings/controller requests:

- Settings are now tabbed: Gameplay, Camera, Keyboard and Controller.
- Controller bindings can be changed by selecting from a dropdown or pressing **Detect** and moving the target button/axis.
- Menus can be navigated with a controller on desktop: left stick/D-pad moves focus, A activates, B backs out, and LB/RB switches settings tabs.
- The pause overlay has a Settings button so players can change local settings while the match is paused.
- Desktop dropdown option text is forced to a light background/dark text so option labels are readable.
- The main menu highlights that controller support works best on PC and shows animated menu-background race cars. Those menu cars are not shown when the game is paused mid-match.

## V36 audio/music notes

V36 adds menu/match music and audio settings.

- Music files live in `public/songs/`:
  - `nitro-boost-dreams.mp3`
  - `turbo-boost-dreams.mp3`
  - `boosting-through-starlight.mp3`
- No Firebase config changes are required for music.
- Signed-in users sync audio preferences through the existing `settings/{uid}` path.
- Browsers require a first tap/click/key press before music or sound effects can start.
